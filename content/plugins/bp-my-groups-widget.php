...
class BP_My_Groups_Widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		<?php if ( bp_has_groups( 'type=active&user_id=' . $bp->loggedin_user->id )&& is_user_logged_in()) : ?>

			<ul id="groups-list-widget" class="item-list">
				<?php while ( bp_groups() ) : bp_the_group(); ?>
					<li>
						...
						<div class="item">
							...
							<div class="item-meta"><span class="activity"><?php printf( __( 'Aktiv for %s', 'buddypress' ), bp_get_group_last_active() ) ?></div>
						</div>
					</li>
		...
		<?php else: ?>
			...
			<div class="widget-error">
				<?php if( is_user_logged_in() ) {
					_e('Du har ikke meldt dig ind i nogen arbejdsgrupper endnu.','buddypress');
				} else {
					_e('Log ind for at se dine grupper.', 'buddypress');
				} ?> 
			</div>
